                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="period" class="form-select form-select-sm">
                            ...
                            <option value="this_week" {{ request('period') == 'this_week' ? 'selected' : '' }}>هذا الأسبوع
                            </option>
                            <option value="this_month" {{ request('period') == 'this_month' ? 'selected' : '' }}>هذا الشهر
                            </option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="text" name="search" class="form-control form-control-sm"
                            placeholder="اسم، هوية، أو جوال..." value="{{ request('search') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">الأرباح من/إلى</label>
                        <div class="input-group input-group-sm">
                            <input type="number" name="min_earnings" class="form-control" placeholder="أدنى"
                                value="{{ request('min_earnings') }}">
                            <input type="number" name="max_earnings" class="form-control" placeholder="أقصى"
                                value="{{ request('max_earnings') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="has_payment_image" class="form-select form-select-sm">
                            ...
                            <option value="0" {{ request('has_payment_image') == '0' ? 'selected' : '' }}>لا يوجد صورة
                            </option>
                        </select>
                    </div>
                    <div class="col-md-3 d-flex align-items-end gap-2">
                        ...
                        <button type="submit" name="export" value="excel" class="btn btn-success btn-sm"
                            title="تصدير Excel"><i class="bi bi-file-earmark-excel"></i></button>
                        <button type="submit" name="export" value="csv" class="btn btn-secondary btn-sm"
                            title="تصدير CSV"><i class="bi bi-file-earmark-spreadsheet"></i></button>
                        <button type="submit" name="export" value="pdf" class="btn btn-danger btn-sm" title="طباعة التقارير"
                            onclick="this.form.target='_blank'; setTimeout(() => this.form.target='', 100);"><i
            ...
            <form action="{{ route('admin.daily-reports.bulk') }}" method="POST" id="bulk-form">
                ...
                <div class="d-flex mb-3 gap-2 align-items-center bg-light p-2 rounded">
                    ...
                    <button type="submit" class="btn btn-primary btn-sm"
                        onclick="return confirm('تأكيد الإجراء الجماعي؟')">تطبيق</button>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th><a
                                        href="?sort=client&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">المندوب</a>
                                </th>
                                <th>رقم المعرف</th>
                                <th><a
                                        href="?sort=city&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">المدينة</a>
                                </th>
                                <th><a
                                        href="?sort=report_date&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">التاريخ</a>
                                </th>
                                <th>المركبة</th>
                                <th><a
                                        href="?sort=earned_amount&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">الأرباح</a>
                                </th>
                                <th><a
                                        href="?sort=completed_orders_count&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">طلبات
                                        مكتملة</a></th>
                                <th><a
                                        href="?sort=rejected_orders_count&direction={{ request('direction') == 'asc' ? 'desc' : 'asc' }}">طلبات
                                        رفض</a></th>
                                <th>صورة</th>
                                <th>الحالة</th>
                                <th>إجراءات</th>
                        ...
                        <tbody>
                            @forelse($reports as $report)
                                <tr>
                                    <td>
                                        <input class="form-check-input report-checkbox" type="checkbox" name="ids[]"
                                            value="{{ $report->id }}">
                                    </td>
                                    ...
                                    <td>
                                        @if($report->payment_image)
                                            <a href="{{ Storage::url($report->payment_image) }}" target="_blank"
                                                class="badge bg-info text-decoration-none">عرض</a>
                                        @else
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('admin.daily-reports.show', $report->id) }}"
                                                class="btn btn-outline-primary" title="التفاصيل"><i class="bi bi-eye"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="12" class="text-center py-4 text-muted">لا توجد تقارير مطابقة للبحث.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
