            ...
            <div class="card border-0 shadow-sm h-100">
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item px-0 d-flex justify-content-between">
                            <span class="text-muted">رقم المعرف:</span>
                        ...
                        <li class="list-group-item px-0 d-flex justify-content-between">
                            ...
                            <span
                                class="badge bg-{{ ($report->client->status ?? '') == 'active' ? 'success' : 'secondary' }}">
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body text-center">
                    @if($report->payment_image)
                        <img src="{{ Storage::url($report->payment_image) }}" alt="كشف الحساب"
                            class="img-fluid rounded border p-1" style="max-height: 500px; object-fit: contain;">
                    @else
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex gap-2 mb-3">
                        <form action="{{ route('admin.daily-reports.status', $report->id) }}" method="POST"
                            class="flex-grow-1">
                            @csrf @method('PUT')
                    ...
                    <form action="{{ route('admin.daily-reports.resubmit', $report->id) }}" method="POST" class="mb-3">
                        ...
                        @if($report->allow_resubmit)
                            <button
                                class="btn btn-secondary w-100 text-start d-flex justify-content-between align-items-center">
                    ...
                    @if($report->payment_image)
                        <a href="{{ Storage::url($report->payment_image) }}" download
                            class="btn btn-info w-100 text-white mb-3 text-start d-flex justify-content-between align-items-center">
                            <span><i class="bi bi-download me-1"></i> تحميل صورة الكشف</span>
                        </a>
                    @endif

                    <form action="{{ route('admin.daily-reports.destroy', $report->id) }}" method="POST"
                        onsubmit="return confirm('هل أنت متأكد من حذف هذا التقرير نهائياً؟');">
                        @csrf @method('DELETE')
                        <button
                            class="btn btn-outline-danger w-100 text-start d-flex justify-content-between align-items-center">
